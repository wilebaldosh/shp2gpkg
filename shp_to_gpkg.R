...
for(i in 1:length(archivos)) {
  ...
  # y localidades como: "Amatl�n de Ca�as", ahora deberá aparecer
  # como "Amatlán de Cañas", entre otros.
  x = st_read(paste0(shp_path, archivos[i]), options ="ENCODING=WINDOWS-1252")

  # Pegamos la ruta y el nombre del archivo, sin extensión,
  # para agregarle la extensión '.gpkg'
  archivo_gpkg <- paste0(paste0(gpkg_path, nombres[i]), '.gpkg')
